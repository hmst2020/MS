############觀察值#################
ob<- data.frame(    # 將觀察值建構data frame 物
  Energy=c(5,6,4,6,5,3),  # 第一行行名與向量值
  Fat=c(9,11,5,9,7,1),    # 第二行行名與向量值
  Calcium=c(20,2,20,46,1,12),  # 第三行行名與向量值
  row.names=c('MC','PR','SL','SD','TN','SH') # 用魚類縮名為各列名
)
############本例函式宣告###########################
...
#############以 R 套件base解題###################
######步驟一. 計算各object觀察值之和################
ob.sum<- cbind(  # 將各營養素依魚類分別加總後於觀察值物件增加一欄
  ob,Sum=unlist(lapply(data.frame(t(ob)),sum))
)
...
ob.sum<- cbind(  # 以初始分群依據將之分為k群
  ...
  cluster=unlist(lapply(
    ...
    function(x){
      y<-floor(k*(x-min.s)/(max.s-min.s)+1)
      ifelse(y>k,k,y)
    })
  )
)
...
for (i in 1:nrow(ob.sum)){ # 計算初始各群組的成員
  x<-ob.sum[i,]
  clust.l[[x$cluster]]<-append(clust.l[[x$cluster]],rownames(ob.sum)[i])
}
print(ob.sum)  # 列印初始分群
...
repeat{
  ...
  for (i in 1:nrow(ob.sum)){
    ...
    for (k in (1:nrow(clust.means))[-cid]){
      ...
      var<- increase-decrease   # 即判斷公式裡的R 值
      if (var<0){    # 若轉移群組有利
